...
def train():
    ...
    # length = [i.split(' ') for i in training_data_text]
    # lenlen = [len(i) for i in length]
    # m = Counter(lenlen)
    # x,y = zip(*m.items())
    ...
    # plt.bar(indexes,y,1)
    # plt.xticks(indexes+width*0.5,x)
    # plt.show()

    # print(sum(lenlen)/len(lenlen))
    # print(max(lenlen))
    # sys.exit()
...
def eval(data_path):
    ...
    print('\n',last_check,'\n')
    # print('the checkpoint:',dir(saver))
    # print(saver._name)
